...
const fromEnv = (config = {}) => {
  ...
  const makePathString = path => (0, _Function.pipe)(path, RA.join(pathDelim));
  ...
  const enumerateChildren = path => core.sync(() => {
    ...
    const filteredKeyPaths = keyPaths.filter(keyPath => {
      for (let i = 0; i < path.length; i++) {
        const pathComponent = (0, _Function.pipe)(path, RA.unsafeGet(i));
...
const fromFlatLoop = (flat, prefix, config, split) => {
  ...
  switch (op._tag) {
    ...
    case OpCodes.OP_MAP_OR_FAIL:
      {
        return core.suspend(() => (0, _Function.pipe)(fromFlatLoop(flat, prefix, op.original, split), core.flatMap(core.forEachSequential(a => (0, _Function.pipe)(op.mapOrFail(a), core.mapError(configError.prefixed(prefix)))))));
      }
    ...
    case OpCodes.OP_PRIMITIVE:
      {
        return (0, _Function.pipe)(pathPatch.patch(prefix, flat.patch), core.flatMap(prefix => (0, _Function.pipe)(flat.load(prefix, op, split), core.flatMap(values => {
          if (values.length === 0) {
            const name = (0, _Function.pipe)(RA.last(prefix), Option.getOrElse(() => "<n/a>"));
    ...
    case OpCodes.OP_SEQUENCE:
      {
        return (0, _Function.pipe)(pathPatch.patch(prefix, flat.patch), core.flatMap(patchedPrefix => (0, _Function.pipe)(flat.enumerateChildren(patchedPrefix), core.flatMap(indicesFrom), core.flatMap(indices => {
          ...
          return (0, _Function.pipe)(core.forEachSequential(indices, index => fromFlatLoop(flat, RA.append(prefix, `[${index}]`), op.config, true)), core.map(chunkChunk => {
    ...
    case OpCodes.OP_HASHMAP:
      {
        return core.suspend(() => (0, _Function.pipe)(pathPatch.patch(prefix, flat.patch), core.flatMap(prefix => (0, _Function.pipe)(flat.enumerateChildren(prefix), core.flatMap(keys => {
          return (0, _Function.pipe)(keys, core.forEachSequential(key => fromFlatLoop(flat, concat(prefix, RA.of(key)), op.valueConfig, split)), core.map(values => {
            if (values.length === 0) {
              return RA.of(HashMap.empty());
            }
            const matrix = values.map(x => Array.from(x));
            return (0, _Function.pipe)(transpose(matrix), RA.map(values => HashMap.fromIterable(RA.zip(RA.fromIterable(keys), values))));
          }));
        })))));
      }
    case OpCodes.OP_ZIP_WITH:
      {
        return core.suspend(() => (0, _Function.pipe)(fromFlatLoop(flat, prefix, op.left, split), core.either, core.flatMap(left => (0, _Function.pipe)(fromFlatLoop(flat, prefix, op.right, split), core.either, core.flatMap(right => {
          ...
          if (Either.isRight(left) && Either.isRight(right)) {
            const path = (0, _Function.pipe)(prefix, RA.join("."));
            const fail = fromFlatLoopFail(prefix, path);
            const [lefts, rights] = extend(fail, fail, (0, _Function.pipe)(left.right, RA.map(Either.right)), (0, _Function.pipe)(right.right, RA.map(Either.right)));
            return (0, _Function.pipe)(lefts, RA.zip(rights), core.forEachSequential(([left, right]) => (0, _Function.pipe)(core.zip(left, right), core.map(([left, right]) => op.zip(left, right)))));
          }
...
const parsePrimitive = (text, path, primitive, delimiter, split) => {
  if (!split) {
    return (0, _Function.pipe)(primitive.parse(text), core.map(RA.of), core.mapError(configError.prefixed(path)));
  }
...
const splitIndexInKeys = (map, unmakePathString, makePathString) => {
  ...
  for (const [pathString, value] of map) {
    const keyWithIndex = (0, _Function.pipe)(unmakePathString(pathString), RA.flatMap(key => Option.match(splitIndexFrom(key), {
      onNone: () => RA.of(key),
