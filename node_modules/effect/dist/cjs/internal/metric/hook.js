...
const histogram = key => {
  ...
  (0, _Function.pipe)(bounds, Chunk.sort(number.Order), Chunk.map((n, i) => {
    boundaries[i] = n;
  }));
...
const summary = key => {
  ...
  const snapshot = now => {
    ...
    return calculateQuantiles(error, sortedQuantiles, (0, _Function.pipe)(Chunk.unsafeFromArray(builder), Chunk.sort(number.Order)));
  };
...
const calculateQuantiles = (error, sortedQuantiles, sortedSamples) => {
  ...
  const resolved = (0, _Function.pipe)(tail, ReadonlyArray.reduce(Chunk.of(resolveQuantile(error, sampleCount, Option.none(), 0, head, sortedSamples)), (accumulator, quantile) => {
