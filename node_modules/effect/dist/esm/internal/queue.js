...
class QueueImpl {
  ...
  get shutdown() {
    return core.uninterruptible(core.withFiberRuntime(state => {
      ...
      return pipe(fiberRuntime.forEachParUnboundedDiscard(unsafePollAll(this.takers), d => core.deferredInterruptWith(d, state.id()), false), core.zipRight(this.strategy.shutdown), core.whenEffect(core.deferredSucceed(this.shutdownHook, void 0)), core.asUnit);
    }));
  }
  get isShutdown() {
...
class BackPressureStrategy {
  ...
  get shutdown() {
    return pipe(core.fiberId, core.flatMap(fiberId => pipe(core.sync(() => unsafePollAll(this.putters)), core.flatMap(putters => fiberRuntime.forEachParUnboundedDiscard(putters, ([_, deferred, isLastItem]) => isLastItem ? pipe(core.deferredInterruptWith(deferred, fiberId), core.asUnit) : core.unit, false)))));
  }
