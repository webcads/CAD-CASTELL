...
export const histogram = key => {
  ...
  pipe(bounds, Chunk.sort(number.Order), Chunk.map((n, i) => {
    boundaries[i] = n;
  }));
...
export const summary = key => {
  ...
  const snapshot = now => {
    ...
    return calculateQuantiles(error, sortedQuantiles, pipe(Chunk.unsafeFromArray(builder), Chunk.sort(number.Order)));
  };
...
const calculateQuantiles = (error, sortedQuantiles, sortedSamples) => {
  ...
  const resolved = pipe(tail, ReadonlyArray.reduce(Chunk.of(resolveQuantile(error, sampleCount, Option.none(), 0, head, sortedSamples)), (accumulator, quantile) => {
    const h = Chunk.unsafeHead(accumulator);
    return pipe(accumulator, Chunk.append(resolveQuantile(error, sampleCount, h.value, h.consumed, quantile, h.rest)));
  }));
  return pipe(resolved, Chunk.map(rq => [rq.quantile, rq.value]));
};
/** @internal */
